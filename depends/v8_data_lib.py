...
def get_options():
    mc_path_default = str(Path.home() / "bitchain")
    ...
    parser.add_argument("-m", "--bitchain", metavar="DIR", default=mc_path_default,
                        help="BitChain path prefix (default: %(default)s)")
    ...
    if not Path(options.bitchain).exists():
        parser.error("{!r}: BitChain path does not exist".format(options.bitchain))

    logger.info("{} - {}".format(logger.name, parser.description))
    logger.info("  Path:     {}".format(options.bitchain))
...
def main():
    ...
    options = get_options()
    os.chdir(str(Path(options.bitchain) / "v8build" / "v8" / "out.gn" / "x64.release"))

...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitChain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitChain
    // Mac and Unix: ~/.bitchain
#ifdef WIN32
    // Windows
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return GetSpecialFolderPath(CSIDL_APPDATA) / "BitChainCold";
    }
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitChain";
#else
    // Mac and Unix
    ...
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return pathRet / ".bitchain-cold";        
    }
    return pathRet / ".bitchain";
#endif
}
...
void mc_CheckDataDirInConfFile()
{
    ...
    mapConfig=new mc_MapStringString;
    if(mc_ReadGeneralConfigFile(mapConfig,NULL,"bitchain",".conf") == 0)
    {
...
int mc_Params::ReadConfig(const char *network_name)
{
    ...
    mc_ReadConfigFile(mc_GetConfigFile(network_name,"bitchain",".conf"),&mapArgs, &mapMultiArgs,"-");    

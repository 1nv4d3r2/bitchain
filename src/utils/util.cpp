...
#include "bitchain/bitchain.h"                                              // MCHN
...
/* MCHN START */
static boost::filesystem::path pathCachedBitChain;
static boost::filesystem::path pathCachedBitChainLog;
...
/* MCHN START */
    ...
    pathCachedBitChain=fs::path(string(mc_gState->m_Params->DataDir()));
    fs::path &path =pathCachedBitChain;
    return path;
...
/* MCHN START */
    fs::path &path =pathCachedBitChainLog;
    if (!path.empty())
        return path;
    path=fs::path(string(mc_gState->m_Params->DataDir(2,1)));
    return pathCachedBitChainLog;
}
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bitchain.pid"));

...
bool PushBitChainVerack(CNode* pfrom, bool fIsVerackack);
...
const string strMessageMagic = "BitChain Signed Message:\n";
...
int SetUpgradedParamValue(const mc_OneBitchainParam *param,int64_t value)
{
    ...
    if(strcmp(param->m_Name,"targetblocktime") == 0)
    {
        ...
        SetBitChainParam("targetblocktime",value);
    }   
...
int BitchainNode_ApplyUpgrades(int current_height)
{
    ...
    SetBitChainParams();            
    if(pBitChainFilterEngine)                                                 // Added from version 20012. 
    {
        pBitChainFilterEngine->SetCallbackNames();
    }
...
/* MCHN START */
        ...
        if(!AcceptBitChainTransaction(tx,view,-1,MC_AMT_DEFAULT,reason, &mandatory_fee, &replay))
        {
            return state.DoS(0,
                             error("AcceptToMemoryPool: : AcceptBitChainTransaction failed %s : %s", hash.ToString(),reason),
                             REJECT_NONSTANDARD, reason);
        }
...
*/
        ...
        CAmount nFees = 0;
        int async_count=0;
        bool fIsLicenseTokenTransfer=Is_BitChainLicenseTokenTransfer(tx);
...
/* MCHN START */        
    if(!fJustCheck)
    {
        ...
        {
            pBitChainFilterEngine->Reset(pindex->nHeight-1,1);
        }
    }
/* MCHN END */        
...
/* MCHN START */        
    if (block.GetHash() == Params().HashGenesisBlock()){        
        ...
        for (unsigned int i = 0; i < block.vtx.size(); i++)
        {
            const CTransaction &tx = block.vtx[i];
            string reason;
            if(!AcceptBitChainTransaction(tx,view,offset,MC_AMT_DEFAULT,reason,NULL,NULL))
            {
...
/* MCHN START */        
            ...
            if(!fJustCheck)
            {
                ...
                {
                    return state.DoS(0,
                                     error("ConnectBlock: : AcceptBitChainTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                     REJECT_INVALID, reason);
                }
            }
/* MCHN END */                    
...
/* MCHN START */        
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (tx.IsCoinBase())
        {
            ...
            if(!fJustCheck)
            {
                if(!AcceptBitChainTransaction(tx,view,coinbase_offset,MC_AMT_DEFAULT,reason,NULL,NULL))
                {
                    return state.DoS(0,
                                     error("ConnectBlock: : AcceptBitChainTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                     REJECT_INVALID, reason);
...
/* MCHN START */    
    ...
    if(pBitChainFilterEngine)
    {
        pBitChainFilterEngine->Reset(old_height-1,0);
    }
...
/*    
    if(pBitChainFilterEngine)
    {
        pBitChainFilterEngine->Reset(pindexNew->nHeight-1,1);
    }
 */ 
    ...
    }
    {
        ...
        if(pBitChainFilterEngine)
        {
            pBitChainFilterEngine->m_CoinsCache=&view;
        }
        bool rv = ConnectBlock(*pblock, state, pindexNew, view);        
        if(pBitChainFilterEngine)
        {
            pBitChainFilterEngine->m_CoinsCache=NULL;
        }
        if(rv)
        {
...
//    if (!fInvalidFound)
    {
        ...
        if(pBitChainFilterEngine)
        {
            pBitChainFilterEngine->Reset(chainActive.Height(),0);
        }
...
/* MCHN START */
    ...
    if(mc_gState->m_WalletMode & MC_WMD_TXS)
    {
        ...
        if(pwalletTxsMain->RollBack(NULL,chainActive.Height()) != MC_ERR_NOERROR)
        {
            ...
            {
                strError="Error: The wallet database is inconsistent. Restart BitChain with -rescan to rebuild the wallet database from the blockchain (can take hours), or with -skipwalletchecks to continue operating anyway";
                return false;
            }
        }
    }
...
/* MCHN END */        
    ...
    if (nCheckLevel >= 4) {
        ...
        while (pindex != chainActive.Tip()) {
            ...
            if(pBitChainFilterEngine)
            {
                pBitChainFilterEngine->m_CoinsCache=&coins;
            }            
...
//    RandAddSeedPerfmon();
    ...
    if (strCommand == "version")
    {
        ...
        pfrom->nBitChainServices=0;
        if (!vRecv.empty())
        {
            vRecv >> pfrom->nBitChainServices;
        }
...
/* MCHN START */
        ...
        {            
            ...
            {
                PushBitChainVerack(pfrom,false);
            }
        }
        else
        {
...
/* MCHN START */
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            pfrom->fDisconnect = !PushBitChainVerack(pfrom,true);
                        }
                        else
                        {
    ...
    else if (strCommand == "verackack")
    {
        if(mc_gState->m_NetworkParams->IsProtocolBitchain())
        {
            ...
            {
                pfrom->fDisconnect |= !PushBitChainVerack(pfrom,false);       // MCHN |= to avoid setting fDisconnect to false
            }
            if(pfrom->fDisconnect)
            {
                mc_gState->m_NetworkState = MC_NTS_SEED_READY;

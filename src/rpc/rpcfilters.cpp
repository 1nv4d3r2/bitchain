...
/*
bool AcceptBitChainTransaction(const CTransaction& tx, 
...
*/    
    ...
    err=pFilterEngine->CreateFilter(test_code.c_str(),filter_main_name,pBitChainFilterEngine->m_CallbackNames[filter_type],worker,strError);
    delete worker;
    if(err)
    {
...
Value listfilters(const Array& params, uint32_t filter_type)
{
    ...
    for(int i=0;i<(int)pBitChainFilterEngine->m_Filters.size();i++)
    {
        Object entry;

        if((filter_list.size() == 0) || 
           (filter_list.find(*(uint256*)pBitChainFilterEngine->m_Filters[i].m_Details.GetTxID()) != filter_list.end()) )
        {
            entry=FilterEntry(pBitChainFilterEngine->m_Filters[i].m_Details.GetTxID(),output_level,filter_type);
            if(entry.size()>0)
            {
                bool take_it=false;
                ...
                if(take_it)
                {
                    bool valid=pBitChainFilterEngine->m_Filters[i].m_CreateError.size() == 0;
                    entry.push_back(Pair("compiled",valid));
                    if(check_approval)
                    {
                        entry.push_back(Pair("approved",mc_gState->m_Permissions->FilterApproved(lpEntity,&(pBitChainFilterEngine->m_Filters[i].m_FilterAddress)) !=0 ));
                        entry.push_back(Pair("address",pBitChainFilterEngine->m_Filters[i].m_FilterAddress.ToString()));
                    }
...
Value getfiltertxid(const Array& params, bool fHelp)
{
    ...
    return pBitChainFilterEngine->m_TxID.ToString();
}
...
Value setfilterparam(const json_spirit::Array& params, bool fHelp)
{
    ...
    if( (param_name == "maxshowndata") )
    {
        if( (params[1].type() == int_type) || (params[1].type() == str_type) )
        {
            ...
            pBitChainFilterEngine->m_Params.m_MaxShownData=nValue;
        }
        else
        {
...
Value testfilter(const vector <uint160>& entities,string filter_code, Value txparam,int vout_in,uint32_t filter_type,string library_code)
{
    ...
    mc_Filter *worker=new mc_Filter;
//    err=pFilterEngine->CreateFilter(filter_code,filter_main_name,pBitChainFilterEngine->m_CallbackNames[filter_type],worker,strError);
    ...
    err=pFilterEngine->CreateFilter(test_code.c_str(),filter_main_name,pBitChainFilterEngine->m_CallbackNames[filter_type],worker,pBitChainFilterEngine->GetSendTimeout(), strError);
    if(err)
    {
        ...
        if(relevant_filter)
        {
            ...
            if ( (filter_type == MC_FLT_TYPE_TX) && all_inputs_found)
            {
...

                {
                    ...
                    CCoinsViewMemPool viewMemPool(pcoinsTip, mempool);
                    view.SetBackend(viewMemPool);
                    if(!AcceptBitChainTransaction(tx,view,-1,MC_AMT_NO_FILTERS,reason,NULL,NULL))
                    {
            ...
            err=pBitChainFilterEngine->RunFilterWithCallbackLog(tx,vout,stream_txid,worker,strError,callbacks);
            if(err)
            {
                errorCode=RPC_INTERNAL_ERROR;

 ...
 */ 
    ...
    {
//        if(pBitChainFilterEngine->m_TxID != 0)
        if(pBitChainFilterEngine->InFilter())
        {
...
/* MCHN START */    
// TODO too many duplicate code with ListWalletTransactions and may be AccepBitChainTransaction
...
int VerifyNewTxForStreamFilters(const CTransaction& tx,std::string &strResult,mc_BitChainFilter **lppFilter,int *applied)            
{
    ...
    if(pBitChainFilterEngine->NoStreamFilters())
    {
        return MC_ERR_NOERROR;
    }
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) 
    {
        ...
        if(result.type() == obj_type)
        {
            ...
            int err=pBitChainFilterEngine->RunStreamFilters(tx,i,(unsigned char*)&hash+MC_AST_SHORT_TXID_OFFSET,-1,0, 
...
//        throw runtime_error("Help message not found\n");
    ...
    if(pBitChainFilterEngine->m_Vout < 0)
    {
    ...
    if(pBitChainFilterEngine->m_Vout >= (int)pBitChainFilterEngine->m_Tx.vout.size())
    {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "vout out of range");                                            
    }
    Value result=DataItemEntry(pBitChainFilterEngine->m_Tx,pBitChainFilterEngine->m_Vout,streams_already_seen, 0x0E00);
...
//    result.get_obj().push_back(Pair("txid", pBitChainFilterEngine->m_Tx.GetHash().GetHex()));
    result.get_obj().push_back(Pair("vout", pBitChainFilterEngine->m_Vout));
...
Value getfilterstream(const Array& params, bool fHelp)
{
    ...
    Value result=StreamEntry((unsigned char*)&(pBitChainFilterEngine->m_EntityTxID),0x0002);
...
*/    
    Object result;
    TxToJSON(pBitChainFilterEngine->m_Tx, 0, result);
...
Value disablerawtransaction(const Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "disablerawtransaction supported only for protocol=bitchain");
    }    

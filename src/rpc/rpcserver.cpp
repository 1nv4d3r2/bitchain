...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "BitChain server stopping";
}
...
void StartRPCThreads(string& strError)
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use bitchaind, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
              ...
              "for example: alertnotify=echo %%s | mail -s \"BitChain Alert\" admin@foo.com\n"),
...
json_spirit::Value CRPCTable::execute(const std::string &strMethod, const json_spirit::Array &params, const Value& req_id) const
{
    ...
    if (!pcmd)
    {
        ...
        {
            throw JSONRPCError(RPC_NOT_SUPPORTED, "Method not available in cold version of BitChain");            
        }
    }
#ifdef ENABLE_WALLET
    if (pcmd->reqWallet && !pwalletMain)                                        // Never happens, reqWallet is changed to require Wallet read lock 
...
//    if(mc_gState->m_ProtocolVersionToUpgrade > mc_gState->m_NetworkParams->ProtocolVersion())
    if( (mc_gState->m_ProtocolVersionToUpgrade > 0) && (mc_gState->IsSupported(mc_gState->m_ProtocolVersionToUpgrade) == 0) )
    {
        ...
        {
            throw JSONRPCError(RPC_UPGRADE_REQUIRED, strprintf("BlockChain was upgraded to protocol version %d, please upgrade BitChain",mc_gState->m_ProtocolVersionToUpgrade));
        }
    }
...
std::string HelpExampleCli(string methodname, string args){
    return "> bitchain-cli " + std::string(mc_gState->m_NetworkParams->Name()) + " " + methodname + " " + args + "\n";
}

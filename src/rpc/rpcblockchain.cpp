...
//        throw JSONRPCError(RPC_INVALID_PARAMS, "Wrong number of parameters");                    
    ...
    if( (vin < 0) || (vin >= (unsigned int)pBitChainFilterEngine->m_Tx.vin.size()) )
    {
    ...
    Array getxoutparams;
    getxoutparams.push_back(pBitChainFilterEngine->m_Tx.vin[vin].prevout.hash.ToString());
    getxoutparams.push_back((int64_t)pBitChainFilterEngine->m_Tx.vin[vin].prevout.n);
...
Value gettxout(const Array& params, bool fHelp)
{
    ...
    {
        if (!((CCoinsViewCache*)(pBitChainFilterEngine->m_CoinsCache))->GetCoins(hash, coins))
            return Value::null;        
    }
    else
    {
        if (fMempool) {
            ...
            if (!view.GetCoins(hash, coins))
                return Value::null;
            if(pBitChainFilterEngine->InFilter() == 0)                                // In filter we already checked this input exists, but mempool is dirty
            {

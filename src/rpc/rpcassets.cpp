    ...
    if (params.size() > 0)
    {
        if (params[0].get_str() != "*") 
        {
            if (params[0].get_str() != "") 
            {
                ...
                {
                    throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need getassetbalances, run bitchaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
                }            
            }
            BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
            {
...
Value getfilterassetbalances_operation(const Array& params, bool fHelp,bool aggregate_tokens)
{
    ...
    for(int j=0;j<(int)pBitChainFilterEngine->m_Tx.vin.size();j++)
    {
        int n=pBitChainFilterEngine->m_Tx.vin[j].prevout.n;
        CCoins coins;
        {
            if(pBitChainFilterEngine->m_CoinsCache)
            {
                if (!((CCoinsViewCache*)(pBitChainFilterEngine->m_CoinsCache))->GetCoins(pBitChainFilterEngine->m_Tx.vin[j].prevout.hash, coins))
                    return Value::null;        
            }
            else
            {
                ...
                if (!view.GetCoins(pBitChainFilterEngine->m_Tx.vin[j].prevout.hash, coins))
                {
                    return Value::null;                                                                 
                }
            }
                    ...
                    for(int i=0;i<asset_amounts->GetCount();i++)
                    {
                        if(aggregate_tokens)
                        {
                            ...
                            else
                            {
                                ...
                                if(memcmp(lpAsset->GetTxID(),&(pBitChainFilterEngine->m_Tx.vin[gin].prevout.hash),sizeof(uint256)) == 0)
                                {
    ...
    for (int j = 0; j < (int)pBitChainFilterEngine->m_Tx.vout.size(); ++j)
    {
        const CTxOut& txout = pBitChainFilterEngine->m_Tx.vout[j];
        if(!txout.scriptPubKey.IsUnspendable())
        {
            CTxDestination address;
            int required=0;
                ...
                if(CreateAssetBalanceList(txout,asset_amounts,lpScript,&required,aggregate_tokens))
                {
                    for(int i=0;i<asset_amounts->GetCount();i++)
                    {
                        if(aggregate_tokens)
                        {
                            ...
                            {
                                if(memcmp(lpAsset->GetTxID(),&(pBitChainFilterEngine->m_TxID),sizeof(uint256)) == 0)
                                {
                                    if( mc_GetABRefType(asset_amounts->GetRow(i)) == MC_AST_ASSET_REF_TYPE_GENESIS )                    
                                    {
                                        quantity=mc_GetABQuantity(asset_amounts->GetRow(i));

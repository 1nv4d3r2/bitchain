...
#include "filters/bitchainfilter.h"

extern mc_BitChainFilterEngine* pBitChainFilterEngine;
...
typedef struct CBitChainTxDetails
{    
...
bool CBitChainTxDetails::IsRelevantInput(int vin, int vout)
{
...
bool CBitChainTxDetails::IsRelevantEntity(uint160 hash)
{
...
void CBitChainTxDetails::SetRelevantEntity(void *entity)
{
    uint160 hash=0;
...
void CBitChainTxDetails::SetAffectedLibrary(void *entity)
{
    uint160 hash=0;
    memcpy(&hash,entity,MC_AST_SHORT_TXID_SIZE);
...
bool mc_CompareAssetQuantities(CBitChainTxDetails *details,string& reason)
{
    unsigned char *ptrIn;
...
void BitChainTransaction_SetTmpOutputScript(const CScript& script1)
{
    CScript::const_iterator pc1 = script1.begin();
    mc_gState->m_TmpScript->Clear();
    mc_gState->m_TmpScript->SetScript((unsigned char*)(&pc1[0]),(size_t)(script1.end()-pc1),MC_SCR_TYPE_SCRIPTPUBKEY);
...
bool BitChainTransaction_CheckCachedScriptFlag(const CTransaction& tx)
{
    bool flag=false;
...
bool BitChainTransaction_CheckCoinbaseInputs(const CTransaction& tx,     
                                               CBitChainTxDetails *details)
{
    if(!tx.IsCoinBase())                                                  
    {   
        return false;
...
bool BitChainTransaction_CheckInputs(const CTransaction& tx,                  // Tx to check
                                       const CCoinsViewCache &inputs,           // Tx inputs from UTXO database
                                       CBitChainTxDetails *details,           // Tx details object
...
bool BitChainTransaction_CheckNewEntity(int vout,
                                          bool& fScriptParsed,
                                          CBitChainTxDetails *details,   
                                          string& reason)      
{
...
bool BitChainTransaction_CheckCachedScript(const CTransaction& tx, 
...
void BitChainTransaction_FillAdminPermissionsBeforeTx(const CTransaction& tx,
                                                        CBitChainTxDetails *details)      
{
...
bool BitChainTransaction_VerifyAndDeleteDataFormatElements(string& reason,int64_t *total_size,uint32_t *salt_size)
{    
...
bool BitChainTransaction_CheckOpReturnScript(const CTransaction& tx, 
                                               const CCoinsViewCache &inputs,   
                                               int vout,
                                               CBitChainTxDetails *details,   
                                               string& reason)      
{
    ...
    total_offchain_size=0;
    if(!BitChainTransaction_VerifyAndDeleteDataFormatElements(reason,&total_offchain_size,NULL))
    {
        return false;
    }
    ...
    if( mc_gState->m_TmpScript->GetNumElements() == 2 )                         // Cached input script or new entity
    {
        ...
        {
                                                                                // Cached scripts
            if(!BitChainTransaction_CheckCachedScript(tx, inputs, fScriptParsed, details, reason))
            {
                return false;
            }
        }
        ...
        {
                                                                                // New entities
            if(!BitChainTransaction_CheckNewEntity(vout, fScriptParsed, details, reason))
            {
                return false;
            }
        }
                                                                                ...
                                                                                // Performed only if it is indeed needed
    {
        ...
        {
            BitChainTransaction_FillAdminPermissionsBeforeTx(tx,details);
        }                        
    }
...
bool BitChainTransaction_CheckAssetUpdateDetails(mc_EntityDetails *entity,
                                                   int vout,
                                                   CBitChainTxDetails *details,   
                                                   string& reason)      
{
...
bool BitChainTransaction_CheckVariableUpdateDetails(const CTransaction& tx,
                                                   mc_EntityDetails *entity,
                                                   int offset, 
                                                   int vout,
                                                   CBitChainTxDetails *details,   
...
bool BitChainTransaction_CheckUpgradeApproval(const CTransaction& tx,
                                                mc_EntityDetails *entity,
                                                int offset,  
                                                int vout,
                                                CBitChainTxDetails *details,   
...
bool BitChainTransaction_CheckStreamItem(mc_EntityDetails *entity,
                                           int vout,
                                           CBitChainTxDetails *details,   
                                           string& reason)      
{
...
bool BitChainTransaction_CheckEntityItem(const CTransaction& tx,
                                           int offset,  
                                           int vout,
                                           CBitChainTxDetails *details,   
                                           string& reason)      
{
    ...
    if(!BitChainTransaction_VerifyAndDeleteDataFormatElements(reason,NULL,&salt_size))
    {
        return false;
    }
       ...
       (entity.GetEntityType() == MC_ENT_TYPE_LIBRARY))                         // Asset, variable or library update
    {
        if(entity.GetEntityType() == MC_ENT_TYPE_ASSET)
        {
            if(!BitChainTransaction_CheckAssetUpdateDetails(&entity,vout,details,reason))
            {
                return false;            
            }
        }
        ...
        {
            if(!BitChainTransaction_CheckVariableUpdateDetails(tx,&entity,offset,vout,details,reason))
            {
                return false;            
            }
        }
    ...
    else
    {
        ...
        {
            if(!BitChainTransaction_CheckUpgradeApproval(tx,&entity,offset,vout,details,reason))
            {
                return false;            
            }
        }
        else                                                                    // (Pseudo)stream item
        {
            if(entity.GetEntityType() <= MC_ENT_TYPE_STREAM_MAX)
            {
                ...
                if(!BitChainTransaction_CheckStreamItem(&entity,vout,details,reason))
                {
                    return false;            
                }            
            }
...
bool BitChainTransaction_CheckDestinations(const CScript& script1,
                                             int vout,
                                             CBitChainTxDetails *details,   
                                             string& reason)      
{
...
bool BitChainTransaction_CheckIfLibraryApproval(unsigned char* ptr,
                                                  CBitChainTxDetails *details)
{
    mc_EntityDetails entity;
...
bool BitChainTransaction_ProcessPermissions(const CTransaction& tx,
                                              int offset,  
                                              int vout,
                            ...
                            if(details->vInputDestinations[i] != 0)
                            {
                                if( ( (type & (MC_PTP_ADMIN | MC_PTP_MINE)) == 0) || details->vInputCanGrantAdminMine[i] || (entity.GetEntityType() > 0) )
                                {
                                            ...
                                            from,to,timestamp,flags,1,offset) == 0)
                                    {
                                        if(type & MC_PTP_FILTER)
                                        {
                                            BitChainTransaction_CheckIfLibraryApproval(ptr,details);
                                        }
                                        fAdminFound=true;
                                    }
                                }
...
bool Is_BitChainLicenseTokenTransfer(const CTransaction& tx)
{
    mc_EntityDetails entity;
    if(tx.IsCoinBase())
    {
    ...
    BitChainTransaction_SetTmpOutputScript(tx.vout[0].scriptPubKey);
...
bool BitChainTransaction_CheckLicenseTokenTransfer(const CTransaction& tx,
                                               int unchecked_row, 
                                               CBitChainTxDetails *details,   
                                               string& reason)      
{
...
bool BitChainTransaction_CheckAssetTransfers(const CTransaction& tx,
                                               int offset,  
                                               int vout,
                                               CBitChainTxDetails *details,   
                                               string& reason)      
{
    ...
    if(!BitChainTransaction_CheckLicenseTokenTransfer(tx,unchecked_row,details,reason))
    {
        return false;
    }
...
bool BitChainTransaction_ProcessTokenIssuance(const CTransaction& tx,
                                                int offset,  
                                                bool accept,
                                                int vout,
...
bool BitChainTransaction_CheckOutputs(const CTransaction& tx,                 // Tx to check
                                        const CCoinsViewCache &inputs,          // Tx inputs from UTXO database
                                        ...
                                        string& reason)                         // Error message
{
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)                  // Basic checks, destinations and simple grants
    {
        ...
        BitChainTransaction_SetTmpOutputScript(tx.vout[vout].scriptPubKey);

        if(mc_gState->m_TmpScript->IsOpReturnScript())                    
        {
            if(!BitChainTransaction_CheckOpReturnScript(tx,inputs,vout,details,reason))
            {
                return false;
            }
        }
        ...
        {
            if(!BitChainTransaction_CheckDestinations(tx.vout[vout].scriptPubKey,vout,details,reason))
            {
                return false;
            }            
            ...
            if(!BitChainTransaction_ProcessPermissions(tx,offset,vout,permission_type,true,details,reason))
            {
                return false;
            }            
        }
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)                  // create, issue, activate grants (requiring pre-tx permissions)
    {
        if(details->vOutputScriptFlags[vout] & (MC_MTX_OUTPUT_DETAIL_FLAG_PERMISSION_CREATE | MC_MTX_OUTPUT_DETAIL_FLAG_PERMISSION_FILTER) )
        {
            ...
            if(!BitChainTransaction_ProcessPermissions(tx,offset,vout,permission_type,false,details,reason))
            {
                return false;
            }                        
        }
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
    {
        ...
        if(details->vOutputScriptFlags[vout] & MC_MTX_OUTPUT_DETAIL_FLAG_OP_RETURN_ENTITY_ITEM)
        {
            ...
            if(!BitChainTransaction_CheckEntityItem(tx,offset,vout,details,reason))
            {
                return false;                
            }
        }                                                                                        
    ...
    if(mc_gState->m_Features->NFTokens())
    {
        for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
        {
            ...
            if(!BitChainTransaction_ProcessTokenIssuance(tx,offset,accept,vout,0,details,reason))
            {
                return false;                
            }        
        }    
...
bool BitChainTransaction_CheckTransfers(const CTransaction& tx,               // Tx to check
                                        int offset,                             // Tx offset in block, -1 if in memppol
                                        CBitChainTxDetails *details,          // Tx details object
                                        string& reason)                         // Error message
{
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
    {
        ...
        if(details->vOutputScriptFlags[vout] & MC_MTX_OUTPUT_DETAIL_FLAG_NOT_OP_RETURN)
        {
            ...
            if(!BitChainTransaction_CheckAssetTransfers(tx,offset,vout,details,reason))
            {
                return false;                
            }
        }        
...
int64_t BitChainTransaction_OffchainFee(int64_t total_offchain_size)            // Total size of offchain items
{
    return (MIN_OFFCHAIN_FEE*total_offchain_size + 999)/ 1000;
}

bool BitChainTransaction_CheckMandatoryFee(CBitChainTxDetails *details,     // Tx details object
                                             int64_t *mandatory_fee,            // Mandatory Fee    
                                             string& reason)                    // Error message
{
    *mandatory_fee = BitChainTransaction_OffchainFee(details->total_offchain_size);
...
bool BitChainTransaction_CheckLicenseTokenDetails(CBitChainTxDetails *details,     // Tx details object
...
bool BitChainTransaction_ProcessAssetIssuance(const CTransaction& tx,         // Tx to check
                                                int offset,                     // Tx offset in block, -1 if in memppol
            ...
            (details->new_entity_type != MC_ENT_TYPE_LIBRARY) )
        {
            ...
            {                    
                if(mc_gState->m_Features->LicenseTokens())
                {
                    ...
                    BitChainTransaction_SetTmpOutputScript(tx.vout[issue_vout].scriptPubKey);
                    if(mc_gState->m_Features->License20010())
                    {
                        if(mc_gState->m_TmpScript->GetNumElements() < 2)
                        {
                    ...
                    if(!BitChainTransaction_CheckLicenseTokenDetails(details,token_address,reason))
                    {
                        return false;                                                                                                                                                        
                    }
    ...
    if(err == 0)
    {
        if(mc_gState->m_Features->NFTokens())
        {
            if(followon_mode & MC_ENT_FOMD_NON_FUNGIBLE_TOKENS)
            {
                ...
                {
                    BitChainTransaction_SetTmpOutputScript(tx.vout[vout].scriptPubKey);
                    if(!BitChainTransaction_ProcessTokenIssuance(tx,offset,accept,vout,txid,details,reason))
                    {
                        return false;                
                    }        
                }
...
bool BitChainTransaction_ProcessEntityCreation(const CTransaction& tx,        // Tx to check
                                                 int offset,                    // Tx offset in block, -1 if in memppol
                                                 bool accept,                   // Accept to mempools if successful
                                                 CBitChainTxDetails *details, // Tx details object
                                                 string& reason)                // Error message
...
//                if( (details->new_entity_type <= MC_ENT_TYPE_STREAM_MAX) ||     // Admin persmission is required for upgrades and filters
    ...
    if(update_mempool)
    {
        if(mc_gState->m_Assets->FindEntityByTxID(&entity,(unsigned char*)&txid))
        {
            ...
            if(details->new_entity_type == MC_ENT_TYPE_FILTER)
            {
                pBitChainFilterEngine->AddFilter((unsigned char*)&txid+MC_AST_SHORT_TXID_OFFSET,(offset < 0) ? 0 : 1);
            }
        }
        else
        {
...
bool BitChainTransaction_VerifyNotFilteredRestrictions(const CTransaction& tx,        // Tx to check
                                                         int offset,                    // Tx offset in block, -1 if in memppol
                                                         bool accept,                   // Accept to mempools if successful
                                                         CBitChainTxDetails *details, // Tx details object
                                                         string& reason)                // Error message
...
bool BitChainTransaction_VerifyStandardCoinbase(const CTransaction& tx,        // Tx to check
                                                        CBitChainTxDetails *details, // Tx details object
                                                        string& reason)                // Error message
{
    ...
    if(signature_output == (mc_gState->m_NetworkParams->IsProtocolBitchain() == 0))
    {
        ...
        return true;                                                            // Signature output should appear only if protocol=bitchain
    }
...
bool BitChainTransaction_ProcessLibraryUpdates(int offset,
                                                 CBitChainTxDetails *details, // Tx details object                                                 
                                                 string& reason)                // Error message
{
    ...
    if(pBitChainFilterEngine)
    {    
        pBitChainFilterEngine->CheckLibraries(&(details->vAffectedLibraries),(offset >= 0));
    }
    return true;
}


bool AcceptBitChainTransaction   (const CTransaction& tx,                     // Tx to check
                                    const CCoinsViewCache &inputs,              // Tx inputs from UTXO database
                                    int offset,                                 // Tx offset in block, -1 if in memppol
                                    ...
                                    uint32_t *replay)                           // Replay flag - if tx should be rechecked or only permissions
{
    ...
    details.fCheckCachedScript=BitChainTransaction_CheckCachedScriptFlag(tx);
    
    mc_gState->m_TmpAssetsIn->Clear();
    
    if(!BitChainTransaction_CheckCoinbaseInputs(tx,&details))                 // Inputs
    {
        if(!BitChainTransaction_CheckInputs(tx,inputs,&details,reason))
        {
            return false;
        }
    }
    ...
    if(!BitChainTransaction_CheckOutputs(tx,inputs,offset,accept,&details,reason))   // Outputs        
    {
        fReject=true;
        goto exitlbl;                                                                    
    }
    
    if(!BitChainTransaction_ProcessAssetIssuance(tx,offset,accept,&details,reason))                // Asset genesis/followon
    {
        fReject=true;
        goto exitlbl;                                                                    
    }

    if(!BitChainTransaction_CheckMandatoryFee(&details,&mandatory_fee,reason))
    {
        fReject=true;
        goto exitlbl;                                                                            
    }                                                                           

    if(!BitChainTransaction_CheckTransfers(tx,offset,&details,reason))        // Transfers and receive permissions
    {
        fReject=true;
        goto exitlbl;                                                                    
    }
                                                                                // Creating of (pseudo)streams/upgrades
    if(!BitChainTransaction_ProcessEntityCreation(tx,offset,accept,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                    
    }

    if(!BitChainTransaction_VerifyNotFilteredRestrictions(tx,offset,accept,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
    
    if(!BitChainTransaction_VerifyStandardCoinbase(tx,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
    
    if(!BitChainTransaction_ProcessLibraryUpdates(offset,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
        ...
        ((flags & MC_AMT_NO_FILTERS) == 0) )
    {
        if(mc_gState->m_Features->Filters())
        {
            if(pBitChainFilterEngine)
            {
                mc_BitChainFilter* lpFilter;
                int applied=0;

                if(pBitChainFilterEngine->RunTxFilters(tx,details.vRelevantEntities,reason,&lpFilter,&applied,(offset >= 0)) != MC_ERR_NOERROR)
                {
...
exitlbl:
    ...
    if(!accept || fReject)                                                      // Rolling back permission database if we were just checking or error occurred    
    {
        ...
        BitChainTransaction_ProcessLibraryUpdates(offset,&details,reason);
    }

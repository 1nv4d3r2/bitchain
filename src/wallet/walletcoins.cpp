...
int64_t BitChainTransaction_OffchainFee(int64_t total_offchain_size);
...
bool FindRelevantCoins(CWallet *lpWallet,                                       // IN  Wallet (to access grouping object)
                       ...
                       unsigned char *in_row,int in_size,                       // TMP temporary buffer row and its size
                       mc_Script *lpScript,                                     // TMP temporary bitchain script object
                       int *in_special_row,                                     // IN  Coordinates of special rows
                      ...
                      const CCoinControl* coinControl)
{
    ...
    if(( nTargetValue > 0) || (!found_selected) )
    {
        ...
        if(!lpWallet->SelectBitChainCoins(nTargetValue,vCoins,in_map,in_amounts,in_special_row[0],in_asset_row,in_prefered_row,setAssetCoins, nAssetValueIn, coinControl))
        {
            return false;
        }                        
                              ...
                              std::string& strFailReason)                       // OUT error message
{
    ...
    if(MIN_OFFCHAIN_FEE)
    {
        ...
        {
            BitChainTransaction_SetTmpOutputScript(s.first);
            mc_gState->m_TmpScript->ExtractAndDeleteDataFormat(NULL,NULL,NULL,&total_offchain_size);            
        }
        mandatory_fee=BitChainTransaction_OffchainFee(total_offchain_size);
    }
...
//    if (vecSend.empty() || nValue < 0)
    ...
    last_time=this_time;
                                                                                ...
                                                                                // We need this as in SelectBitChainCoinsMinConf coins will be shuffled
...
bool CWallet::CreateBitChainTransaction(const vector<pair<CScript, CAmount> >& vecSend,
                                CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl,

...
uint64_t nLocalBitChainServices = 0;
...
/* MCHN START */
    ...
    nLocalBitChainServices |= pEF->NET_Services();
    if(mc_gState->m_NetworkParams->IsProtocolBitchain())
    {
        subver=FormatSubVersion("BitChain", mc_gState->GetProtocolVersion(), std::vector<string>());
    }
    PushMessage("version", PROTOCOL_VERSION, nLocalServices, nTime, addrYou, addrMe,
                nLocalHostNonce, subver, nBestHeight, true, nLocalBitChainServices);
/* MCHN END */
}
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "BitChain " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. BitChain Core is probably already running."), addrBind.ToString());
        else

...
int mc_BitChainFilterParams::Zero()
{
    m_MaxShownData=-1;
...
int mc_BitChainFilterParams::Init()
{
    Zero();
...
int mc_BitChainFilterParams::Close()
{
    mc_gState->m_Compatibility=m_Compatibility;    
    return MC_ERR_NOERROR;
}

int mc_BitChainFilterParams::Destroy()
{
    return Zero();
}
...
int mc_BitChainLibrary::Destroy()
{
    Zero();
...
int mc_BitChainLibrary::Initialize(const unsigned char* short_txid,uint32_t index)
{
    return MC_ERR_NOERROR;    
}

int mc_BitChainFilter::Zero()
{
...
int mc_BitChainFilter::Initialize(const unsigned char* short_txid)
{
    size_t value_size;
    unsigned char *ptr;
    ...
    if(ptr)
    {
        m_FilterCodeRow=pBitChainFilterEngine->m_CodeLibrary->GetNumElements();
        pBitChainFilterEngine->m_CodeLibrary->AddElement();
        pBitChainFilterEngine->m_CodeLibrary->SetData(ptr,value_size);
        pBitChainFilterEngine->m_CodeLibrary->SetData(&ntc,1);        
    }                                    
    else
    {
        m_FilterCodeRow=pBitChainFilterEngine->m_CodeLibrary->GetNumElements();
...
bool mc_BitChainFilter::HasRelevantEntity(set <uint160>& sRelevantEntities)
{
...
int mc_BitChainFilterEngine::Zero()
{
    m_Filters.clear();
    m_TxID=0;
    m_EntityTxID=0;
...
int mc_BitChainFilterEngine::Destroy()
{
    if(m_Semaphore)
    {
...
void mc_BitChainFilterEngine::Lock(int write_mode)
{        
...
void mc_BitChainFilterEngine::UnLock()
{    
    m_LockedBy=0;
    __US_SemPost(m_Semaphore);
}

int mc_BitChainFilterEngine::InFilter()
{    
...
int mc_BitChainFilterEngine::GetAcceptTimeout()
{    
    return GetArg("-acceptfiltertimeout",DEFAULT_ACCEPT_FILTER_TIMEOUT);
}

int mc_BitChainFilterEngine::GetSendTimeout()
{    
...
int mc_BitChainFilterEngine::SetTimeout(int timeout)
{
    Lock(1);
...
int mc_BitChainFilterEngine::SetTimeoutInternal(int timeout)
{
    for(int i=0;i<(int)m_Filters.size();i++)
    {
        mc_Filter *worker=*(mc_Filter **)m_Workers->GetRow(i);
...
uint160 mc_BitChainFilterEngine::ActiveUpdateID(uint160 hash)
{
    map<uint160,mc_BitChainLibrary>::iterator it=m_Libraries.find(hash);
...
int mc_BitChainFilterEngine::LoadLibrary(uint160 hash,bool *modified)
{
    int count;
    size_t value_size;
    unsigned char *ptr;
    ...
    for(int i=0;i<count;i++)
    {
        ...
        map<uint160,mc_BitChainLibrary>::iterator it=m_Libraries.find(library[i].m_Hash);
        if (it == m_Libraries.end())
        {
            if(i == 0)
            {
...
int mc_BitChainFilterEngine::CheckLibraries(set<uint160>* lpAffectedLibraries, int for_block)
{
    Lock(1);
...
int mc_BitChainFilterEngine::CheckLibrariesInternal(set<uint160>* lpAffectedLibraries, int for_block)
{
    int err;
    bool modified;
    map<uint160,mc_BitChainLibrary>::iterator mit;
    map<uint160,mc_BitChainLibrary>::iterator lit;    
...
mc_Filter *mc_BitChainFilterEngine::StreamFilterWorker(int row,bool *modified)
{
    int err; 
    char *code;
    size_t code_size;
    ...
    *modified=false;
    ...
    for (unsigned int i=0;i<m_Filters[row].m_Libraries.size();i++) 
    {
        ...
        map<uint160,mc_BitChainLibrary>::iterator it=m_Libraries.find(mc_LibraryIDForUpdate(m_Filters[row].m_Libraries[i]));
        if(it == m_Libraries.end())
        {
...
int mc_BitChainFilterEngine::RebuildFilter(int row,int for_block)
{
    int err; 
    ...
    for (unsigned int i=0;i<m_Filters[row].m_Libraries.size();i++) 
    {
        map<uint160,mc_BitChainLibrary>::iterator it=m_Libraries.find(m_Filters[row].m_Libraries[i]);
        if (it != m_Libraries.end())
        {
...
int mc_BitChainFilterEngine::AddFilter(const unsigned char* short_txid,int for_block)
{
    Lock(1);
...
int mc_BitChainFilterEngine::AddFilterInternal(const unsigned char* short_txid,int for_block)
{    
    int err;
    mc_BitChainFilter filter;
...
int mc_BitChainFilterEngine::Reset(int block,int for_block)
{
    Lock(1);
...
int mc_BitChainFilterEngine::ResetInternal(int block,int for_block)
{
    int filter_block;
    int err;
...
int mc_BitChainFilterEngine::NoStreamFilters()
{
    int ret=1;
...
int mc_BitChainFilterEngine::RunStreamFilters(const CTransaction& tx,int vout, unsigned char *stream_short_txid,int block,int offset,std::string &strResult,mc_BitChainFilter **lppFilter,int *applied)            
{
    if(mc_gState->m_Features->StreamFilters() == 0)
    {
        return MC_ERR_NOERROR;
...
int mc_BitChainFilterEngine::RunTxFilters(const CTransaction& tx,std::set <uint160>& sRelevantEntities,std::string &strResult,mc_BitChainFilter **lppFilter,int *applied,bool only_once)
{    
    Lock(0);
...
int mc_BitChainFilterEngine::RunFilter(const CTransaction& tx,mc_Filter *filter,std::string &strResult)
{
    Lock(0);
    int err=MC_ERR_NOERROR;
    m_Tx=tx;
...
int mc_BitChainFilterEngine::RunFilterWithCallbackLog(const CTransaction& tx,int vout,uint256 stream_txid,mc_Filter *filter,std::string &strResult, json_spirit::Array& callbacks)
{
    Lock(0);
    int err=MC_ERR_NOERROR;
    m_Tx=tx;
...
void mc_BitChainFilterEngine::SetCallbackNames()
{
    Lock(1);
...
void mc_BitChainFilterEngine::SetCallbackNamesInternal()
{
    m_CallbackNames.clear();
...
int mc_BitChainFilterEngine::Initialize()
{
    mc_Buffer *filters;
    unsigned char *txid;
    int err=MC_ERR_NOERROR;

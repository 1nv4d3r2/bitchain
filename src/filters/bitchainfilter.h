...
#ifndef BITCHAINFILTER_H
#define BITCHAINFILTER_H
...
#define MC_FLT_MAIN_NAME_TEST              "_bitchain_library_test_"
...
typedef struct mc_BitChainFilter
{
...
typedef struct mc_BitChainLibrary
{
    mc_EntityDetails m_Details;
    std::string m_CreateError;
...
typedef struct mc_BitChainFilterParams
{
    int m_MaxShownData;
    int m_Compatibility;
    mc_BitChainFilterParams()
    {
        Zero();
    }
...
typedef struct mc_BitChainFilterEngine
{
    std::vector <mc_BitChainFilter> m_Filters;
    std::vector <std::vector <std::string>> m_CallbackNames;
    std::map<uint160,mc_BitChainLibrary> m_Libraries;
    ...
    CTransaction m_Tx;
    int m_Vout;
    mc_BitChainFilterParams m_Params;
    void *m_CoinsCache;
    
    void *m_Semaphore;
    uint64_t m_LockedBy;
    
    mc_BitChainFilterEngine()
    {
        Zero();
    }
    ...
    int RunTxFilters(const CTransaction& tx,std::set <uint160>& sRelevantEntities,std::string &strResult,mc_BitChainFilter **lppFilter,int *applied,bool only_once);            
    int RunStreamFilters(const CTransaction& tx,int vout, unsigned char *stream_short_txid,int block,int offset,std::string &strResult,mc_BitChainFilter **lppFilter,int *applied);            

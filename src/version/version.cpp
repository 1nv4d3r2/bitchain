...
int mc_State::VersionInfo(int version)
{
    ...
    if(version < BITCHAIN_VERSION_CODE_MIN_VALID)
    {
        switch(version)
        {
            case BITCHAIN_VERSION_CODE_BUILD:                                 // last version
                return -this_build;                                               
            case BITCHAIN_VERSION_CODE_PROTOCOL_THIS:                         // last protocol version
                return this_protocol;
            case BITCHAIN_VERSION_CODE_PROTOCOL_MIN:
                return 10008;                                                   // first supported version
            case BITCHAIN_VERSION_CODE_PROTOCOL_MIN_DOWNGRADE:
                return 10008;                                                   // cannot downgrade below this version
            case BITCHAIN_VERSION_CODE_PROTOCOL_MIN_NO_DOWNGRADE:
                return 20002;                                                   // if we are on this version or above, downgrades are forbidden
            case BITCHAIN_VERSION_CODE_PROTOCOL_FOR_RELEVANCE:                
    ...
    return VersionInfo(BITCHAIN_VERSION_CODE_BUILD)-1;                        // Created by the following builds
}
...
int mc_State::IsDeprecated(int version)
{
    ...
    if(ptr)
    {
        if(strcmp((char*)ptr,"bitchain"))
        {
            is_protocol_bitchain=0;
        }
    }
    if(is_protocol_bitchain)
    {
        if(build > 0)
        {
...
{
    return -VersionInfo(BITCHAIN_VERSION_CODE_BUILD);
}

int mc_State::GetProtocolVersion()
{
    return VersionInfo(BITCHAIN_VERSION_CODE_PROTOCOL_THIS);
}

int mc_State::MinProtocolVersion()
{
    return VersionInfo(BITCHAIN_VERSION_CODE_PROTOCOL_MIN);
}

int mc_State::MinProtocolDowngradeVersion()
{
    return VersionInfo(BITCHAIN_VERSION_CODE_PROTOCOL_MIN_DOWNGRADE);
}

int mc_State::MinProtocolForbiddenDowngradeVersion()
{
    return VersionInfo(BITCHAIN_VERSION_CODE_PROTOCOL_MIN_NO_DOWNGRADE);    
}

int mc_State::RelevantParamProtocolVersion()
{
    return VersionInfo(BITCHAIN_VERSION_CODE_PROTOCOL_FOR_RELEVANCE);        
}
